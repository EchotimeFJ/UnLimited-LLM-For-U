...
RETRY_DELAY=3
USE_MIRROR=true
USE_BAIDU_MIRROR=true  # 添加百度镜像作为备选
# ==================== 配置区域 ====================
...
get_mirrored_url() {
    local URL="$1"
    local MIRROR_TYPE="${2:-hf}"  # hf 或 baidu
    
    if [ "$USE_MIRROR" = true ]; then
        if [[ "$URL" == *"huggingface.co"* ]]; then
            # 提取仓库路径
            local repo_path=$(echo "$URL" | sed -E 's|https?://huggingface\.co/||')
            
            if [ "$MIRROR_TYPE" = "baidu" ]; then
                # 百度云镜像
                echo "https://hf-mirror.com/${repo_path}"
            else
                # HuggingFace镜像
                echo "https://hf-mirror.com/${repo_path}"
            fi
...
# ----------------------------------------------------------------
# Helper: 获取模型的可用的下载URL列表（按优先级排序）
# ----------------------------------------------------------------
get_model_urls() {
    local original_url="$1"
    
    # 返回所有可用的URL，按优先级排序
    echo "$original_url"  # 原始URL（最后尝试）
    echo "$(get_mirrored_url "$original_url" "hf")"  # HF镜像
}

# ----------------------------------------------------------------
# Helper: Download with resume and multi-source support
# ----------------------------------------------------------------
download_with_resume() {
    ...
    local NAME="$4"
    local RETRY_COUNT=0
    local ATTEMPT_SOURCE=0
    
    mkdir -p "$(dirname "$DEST")"
    
    # 获取所有可用的下载源
    mapfile -t DOWNLOAD_SOURCES < <(get_model_urls "$URL")
    NUM_SOURCES=${#DOWNLOAD_SOURCES[@]}
    
    while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
        RETRY_COUNT=$((RETRY_COUNT + 1))
        
        # 轮询不同的下载源
        local SOURCE_URL="${DOWNLOAD_SOURCES[$ATTEMPT_SOURCE]}"
        local SOURCE_NAME="原始源"
        
        if [[ "$SOURCE_URL" == *"hf-mirror.com"* ]]; then
            SOURCE_NAME="HF镜像"
        fi
        
        echo ""
        echo -e "${CYN}[尝试 $RETRY_COUNT/$MAX_RETRIES] 正在下载 $NAME ($SOURCE_NAME)...${RST}"
        ...
        if [ -f "$DEST" ] && [ $(stat -c%s "$DEST" 2>/dev/null || echo 0) -gt 0 ]; then
            ...
            local EXISTING_MB=$(awk "BEGIN {printf \"%.2f\", $EXISTING_SIZE/1048576}")
            echo -e "${DGR}      已下载 ${EXISTING_MB} MB，继续下载...${RST}"
            CONTINUE_FLAG="-C $EXISTING_SIZE"
        fi
        
        # 执行下载
        if curl -L --retry 3 --retry-delay 2 \
            -f -o "$DEST" $CONTINUE_FLAG "$SOURCE_URL" 2>&1; then
            
            local ACTUAL_SIZE=$(stat -c%s "$DEST" 2>/dev/null || echo 0)
            if [ "$ACTUAL_SIZE" -gt "$MIN_SIZE" ]; then
                local SIZE_GB=$(awk "BEGIN {printf \"%.2f\", $ACTUAL_SIZE/1073741824}")
                echo ""
                echo -e "${GRN}[✓] $NAME 下载成功！ ($SIZE_GB GB)${RST}"
                return 0
            fi
        fi
        
        # 下载失败，切换到下一个源
        ATTEMPT_SOURCE=$(( (ATTEMPT_SOURCE + 1) % NUM_SOURCES ))
        
        if [ $RETRY_COUNT -lt $MAX_RETRIES ]; then
            echo ""
            echo -e "${YLW}[$RETRY_COUNT/$MAX_RETRIES] 下载失败，${RETRY_DELAY}秒后重试（切换下载源）...${RST}"
            sleep $RETRY_DELAY
        fi
    done
    
    echo ""
    echo -e "${RED}[✗] $NAME 下载失败（已重试 $MAX_RETRIES 次）${RST}"
    echo -e "${DGR}      建议：可以手动从浏览器下载模型文件，放到 Shared/models/ 目录${RST}"
    return 1
}
...
echo ""
echo -e "${CYN}==========================================================${RST}"
echo -e "${CYN}   PORTABLE AI USB - 优化版安装程序 (Linux)              ${RST}"
echo -e "${CYN}   [多源下载 + 断点续传 + 智能切换]${GRN}${RST}"
echo -e "${CYN}==========================================================${RST}"
echo ""

echo -e "  使用镜像加速: $(if [ "$USE_MIRROR" = true ]; then echo '是 (hf-mirror.com)'; else echo '否'; fi)"
echo -e "  多源备选: $(if [ "$USE_BAIDU_MIRROR" = true ]; then echo '启用'; else echo '禁用'; fi)"
echo ""
...
# ================================================================
# STEP 4: Download AI models (with resume and multi-source support)
# ================================================================
echo ""
echo -e "${YLW}[4/7] Downloading AI Model(s)...${RST}"
echo -e "${CYN}      智能多源下载，自动切换最快的源${RST}"
...
download_model() {
    ...
    echo -e "${MAG}      下载中... 支持多源切换和断点续传，请勿关闭窗口！${RST}"

    if download_with_resume "$URL" "$DEST" "$MINB" "$NAME"; then
        echo -e "${GRN}      Download complete!${RST}"
    else
        DOWNLOAD_ERRORS+=("$NAME")
        echo -e "${RED}      ERROR: Download failed for ${NAME}!${RST}"
        echo -e "${DGR}      提示: 可以手动从以下地址下载:${RST}"
        echo -e "${DGR}      ${URL}${RST}"
        echo -e "${DGR}      下载后将文件放到: ${MODELS_DIR}/${RST}"
    fi
}
...
if $HAS_CUSTOM && [ -n "$CUSTOM_URL" ]; then
    ...
    if file_ok "$DEST" 100000000; then
        ...
        echo -e "${MAG}      下载自定义模型...${RST}"
        if download_with_resume "$CUSTOM_URL" "$DEST" 100000000 "Custom: $CUSTOM_FILE"; then
            echo -e "${GRN}      Download complete!${RST}"
        else
...
# ================================================================
# STEP 6: Download Ollama Linux engine (with resume support)
# ================================================================
echo ""
echo -e "${YLW}[6/7] Downloading Ollama AI Engine (Linux)...${RST}"
echo -e "${CYN}      使用HuggingFace镜像加速${RST}"
...
if [ -f "$OLLAMA_BIN" ] && is_elf "$OLLAMA_BIN" && file_ok "$OLLAMA_BIN" 10000000; then
    ...
    OLLAMA_URL_ORIGINAL="https://github.com/ollama/ollama/releases/latest/download/ollama-linux-amd64.tar.zst"
    OLLAMA_URL_MIRROR="$HF_MIRROR/ollama/ollama/releases/latest/download/ollama-linux-amd64.tar.zst"
    ...
    while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
        RETRY_COUNT=$((RETRY_COUNT + 1))
        echo ""
        echo -e "${CYN}[尝试 $RETRY_COUNT/$MAX_RETRIES] 正在下载 Ollama 引擎...${RST}"
        
        # 优先使用镜像
        local OLLAMA_URL="$OLLAMA_URL_MIRROR"
        if [ $RETRY_COUNT -gt 2 ]; then
            OLLAMA_URL="$OLLAMA_URL_ORIGINAL"
        fi
        
        CONTINUE_FLAG=""
        if [ -f "$OLLAMA_BIN" ] && is_elf "$OLLAMA_BIN"; then
            rm -f "$OLLAMA_BIN"
            CONTINUE_FLAG="-C -"
        fi
        
        curl -L --retry 3 --retry-delay 2 $CONTINUE_FLAG \
            --fail -o /tmp/ollama-linux.tar.zst "$OLLAMA_URL" 2>&1
        
        if [ $? -eq 0 ]; then
            tar --use-compress-program=zstd -xf /tmp/ollama-linux.tar.zst -C "$SHARED_BIN" \
        ...
        if [ $RETRY_COUNT -lt $MAX_RETRIES ]; then
            echo -e "${YLW}[$RETRY_COUNT/$MAX_RETRIES] 下载失败，${RETRY_DELAY}秒后重试...${RST}"
            sleep $RETRY_DELAY
        fi
    done
...
if [ "${#DOWNLOAD_ERRORS[@]}" -gt 0 ]; then
    ...
    echo -e "${DGR}  Or manually download and place in Shared/models/${RST}"
else
